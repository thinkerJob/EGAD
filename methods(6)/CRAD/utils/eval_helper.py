...
def merge_together(save_dir):
    ...
    print("----------------------------len(preds)---------------------------",len(preds))              #224
    #print("----------------------------preds---------------------------", preds)
    file_names = [info['filename'] for info in fileinfos]
    #print("--------------------------------fileinfos----------------------------",file_names)
    #print("--------------------------------masks----------------------------", masks)                    #全0
...
class EvalImage:
    ...
    def eval_auc(self):
        print("想要分数len(self.preds)-----------------------------------------------------:", len(self.preds))    #长度正确
        #print("想要分数preds-----------------------------------------------------:", self.preds)            #有分数
...
def performances(fileinfos, preds, masks, config):
    ...
    file_names = [info['filename'] for info in fileinfos]
    # print("名字fileinfos------------------------------------------------------------:", fileinfos)         #有地址，是乱的
    # print("分数auc-----------------------------------------------------------:", pred)               #有分数是按类分的
