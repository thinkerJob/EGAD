...
class MVTecDataset(Dataset):
    def __init__(self, c, is_train=True):
        ...
        if is_train:
            self.transform_x = T.Compose([
                T.Resize(c.img_size, Image.ANTIALIAS),
               # T.Resize(c.img_size, Image.Resampling.LANCZOS),    #修改
                T.RandomRotation(5),
                T.CenterCrop(c.crp_size),
                T.ToTensor()])
        # test:
    ...
    def __getitem__(self, idx):
        ...
        if self.class_name in ['carpet', 'bottle', 'hazelnut', 'leather', 'cable', 'capsule', 'grid', 'pill',
            ...
            #print(f"Original shape: {x.shape}")  # 输出原始形状
            if x.ndim == 2:  # 如果是二维（灰度图）
                #print("Channels: ['Gray']")
                x = np.expand_dims(np.array(x), axis=2)
                x = np.concatenate([x, x, x], axis=2)

            if x.ndim == 3 and x.shape[2] == 2:  # 两个通道
                print("Channels: ['Channel 1', 'Channel 2----------------------------------------------']")
                # 例如，对这两个通道求平均值以获取一个单通道图像
                x = np.mean(x, axis=2)  # 生成一个 (500, 500) 的单通道图像
                x = np.stack([x] * 3, axis=2)  # 转换为RGB格式
            ...
            # 确保是三通道或灰度图（被扩展为三通道）
            if x.ndim != 3 or x.shape[2] not in [3]:
                raise ValueError(f"Unsupported image dimension or channel: {x.shape}")
