...
def train(c):
    ...
    for epoch in range(c.meta_epochs):
        ...
        # [height, width, test_image_list, test_dist, gt_label_list, gt_mask_list] = aaa
        ...
        for l, p in tqdm(enumerate(pool_layers)):
            ...
            test_prob = torch.exp(
                torch.tensor(test_dist[l], dtype=torch.double) - torch.max(
                    torch.tensor(test_dist[l], dtype=torch.double)
                )
            )
            test_mask = test_prob.reshape(-1, height[l], width[l])
            # upsample
            ...
            del test_prob, test_mask

        print("score aggregation start")
        # score aggregation
        ...
        # 避免分母为零的情况，可以检查并设置一个小的 epsilon
        epsilon = 1e-8
        range_score = max_score - min_score
        # 归一化处理
        normalized_score = (score_label - min_score) / (range_score + epsilon)
        ...
        #print("想要名字",test_dataset.x)
        #print("想要分数",score_label)
