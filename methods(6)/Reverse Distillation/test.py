...
def test(_class_):
    ...
    test_path = './MVTec-AD_test/' + _class_
    ...
    pr_list_sp= evaluation(encoder, bn, decoder, test_dataloader, device, _class_)
    # 获取所有 img_paths
    img_paths = test_data.img_paths
    pr_list_sp = (pr_list_sp - np.min(pr_list_sp)) / (np.max(pr_list_sp) - np.min(pr_list_sp))
    ...
    file_names = [path.split("MVTec-AD_test/")[-1] for path in img_paths]
    with open("anomaly_scores_with_names.txt", 'a') as f:
        ...
        # f.write(f"{category}\n")
        #print("category",category)  # 输出: bottle
        for name, score in zip(file_names, pr_list_sp):
...
def compute_pro(masks: ndarray, amaps: ndarray, num_th: int = 200) -> None:
    ...
    for th in np.arange(min_th, max_th, delta):
        ...
        #修改   df = df.append({"pro": mean(pros), "fpr": fpr, "threshold": th}, ignore_index=True)
